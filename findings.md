...
- Batch-hard Triplet Loss 以余弦距离选择每个 anchor 的最难同类和最难异类样本。公平 500 步筛选中，权重 0.05/0.10/0.20 的 Top-1 为 56.84%/56.90%/57.00%，均超过无 Triplet 的同阶段 56.78%，选择 0.20 完整训练。
- 首轮短程筛选错误地让 500 步任务使用 500 步余弦周期，而历史对照来自 2000 步周期的第 500 步。学习率分别已降为 0 与仍为 0.000256，结果不可直接比较；新增 `--scheduler-steps` 后重新运行，未用错误口径选型。
- Triplet 权重 0.20 的完整模型 validation 单图登记 Top-1/Top-3 为 60.59%/70.46%，较纯 SupCon 残差模型提高约 0.16/0.16 个百分点。固定 query 的 1/2/3 图登记 Top-1 为 62.08%/71.63%/76.99%，分别提高约 0.20/0.17/0.15 个百分点。
- 提升跨单图和多图协议保持一致，但幅度继续缩小，说明冻结 embedding 上的轻量适配器接近当前上限。是否局部解冻主干应以资源预算和严格 validation 门槛决定，test 继续冻结。
- 队长 ArcFace V2 在相同原始 validation 上的标准 1/2/3 图登记 Top-1 为 59.21%/70.02%/77.12%，Top-3 为 68.36%/78.65%/84.74%；当前 Triplet 对应为 60.59%/70.69%/76.99% 和 70.46%/79.48%/84.59%。1～2 图由 Triplet 胜出，3 图 ArcFace V2 仅领先约 0.13/0.15 个百分点。
- 固定同一批 8,283 个身份和 16,794 个 query 的 matched cohort 后结论不变，说明差异不是由不同 K 的样本群变化造成。校园 MVP 初期优先 Triplet，ArcFace V2 可保留为多图候选。
- 跨身份近重复候选的 178 个关联组中，46 组最小 dHash 距离为 0，25 组为 1–2，107 组为 3–4。第 133 组的五个身份存在多对画面相同或高度一致的图片，第一轮目视判断强烈疑似同猫多 ID；但尚未逐张核对全部照片，不能自动合并标签。第 10 组跨度达 14 个身份，尤其要防止传递关系误合并。
