...
19. [下一步] 校园 MVP 默认采用 Triplet 模型并优先收集 2～3 张登记照；未知猫阈值仍需独立校园实拍样本校准，当前不自动确认身份。
20. [已完成] 仅在 validation 内建立身份不重叠的未知猫阈值校准/核验实验；排除疑似标签冲突身份，不读取最终 test。1% 未知猫误接受目标下已知猫正确接受仅 30.94%，暂不部署自动阈值。
